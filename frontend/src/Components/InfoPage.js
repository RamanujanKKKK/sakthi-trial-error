...
const InfoPage = (props) => {
    ...
    if (filterDept.includes(dept_id) || filterDept.length == 0) {
      ...
      props.data.employeebyDept[dept_id].forEach((emp) => {
        ...
        emp.training.forEach((training_id) => {
          let training = props.dataHandler.getTrainingById(
            props.data,
            training_id
          );
          if (!training) return;
          if (
            ...
            filterTraining.includes(training_id)
          ) {
  ...
  return (
    <div style={{ padding: "10px" }}>
      <div>
        <div className="heading-infopage">Employee Filter</div>
        <div className="filter-info">
          <Dropdown
            name={"Trainings"}
            data={props.data.training}
            addData={addTraining}
            removeData={removeTraining}
          />
          <Dropdown
            name={"Departments"}
            data={props.data.department}
            addData={addDept}
            removeData={removeDept}
          />
        </div>
        <div className="table">
