...
const ScheduleView = (props) => {
  ...
  props.data.schedule.forEach((element) => {
    if (
      props.data.filter.training.filter((ele) => ele.id == element.name_id) &&
      props.fromDate <= element.from_date &&
      props.toDate >= element.to_date
    ) {
      let outD = clone(element);
      outD.training = props.dataHandler.getTrainingById(
        props.data,
        element.name_id
      ).name;
      let trainer = props.dataHandler.getTrainerById(
        props.data,
        element.trainer_id
      );
