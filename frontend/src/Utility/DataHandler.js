...
class DataHandler {
  ...
  removeTraining(data, training) {
    data = this.clone(data);
    data.filter.training = data.filter.training.filter(
      (ele) => ele.name != training.name
    );
    this.dataHandler(data);
  }
  removeDepartment(data, dept) {
    data = this.clone(data);
    data.filter.department = data.filter.department.filter(
      (ele) => ele.name != dept.name
    );
  ...
  inverseFilter(data) {
    data = this.clone(data);
    data.filter.training = data.training.filter(
      (x) => data.filter.training.filter((y) => y.id == x.id).length == 0
    );
    data.filter.department = data.department.filter(
      (x) => data.filter.department.filter((y) => y.id == x.id).length == 0
    );
    this.dataHandler(data);
  }
  emptyFilter(data) {
